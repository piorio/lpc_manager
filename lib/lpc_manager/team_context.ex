defmodule LpcManager.TeamContext do
  ...
  def validate_reroll_number(%LpcManager.TeamContext.Team{roster_team: roster_team}, %{
        "re_roll" => bought_reroll_string
      }) do
    {bought_reroll, _} = Integer.parse(bought_reroll_string)
    bought_reroll <= roster_team.re_roll_max
  end

  def validate_apothecary(%LpcManager.TeamContext.Team{roster_team: roster_team}, %{
  ...
  def get_money_spent(
        %LpcManager.TeamContext.Team{roster_team: roster_team, treasury: treasury},
        teams_param
      ) do
    apothecary_spent =
      case String.to_existing_atom(teams_param["apothecary"]) do
        true -> 50000
        false -> 0
      end
    ...
    spent =
      re_roll_spent * roster_team.re_roll_cost + assistan_coach_spent * 10000 +
        cheerleader_spent * 10000
